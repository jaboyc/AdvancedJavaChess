...
/**
 * Handles Zobrist Key management. Applied from https://github.com/ucarion/godot/blob/master/src/zobrist/Zobrist.java#L150
 */
public class ZobristKey {

    /**
     * Hashes for all the pieces.
     */
    ...
    public static final long whiteRightCastling = 0x31d71dce64b2c310L;
    public static final long whiteLeftCastling = 0xf165b587df898190L;
    public static final long blackRightCastling = 0xa57e6339dd2cf3a0L;
    public static final long blackLeftCastling = 0x1ef6e6dbb1961ec9L;
    ...
    /**
     * Returns the key for the given square.
     *
     * @param pos   the position to get the key for.
     * @param piece the piece at the square.
     * @return the key for the square.
     */
    public static long getKeyForSquare(int pos, int piece) {
        switch (piece) {
            case PAWN:
                return pawn[0][pos];
            case -PAWN:
    ...
    /**
     * Returns the key for a move.
     *
     * @param move  the move to get the key from.
     * @param board the board to get the key from.
     ...
     * @return the key for the given board.
     */
    public static long getKeyForBoard(Board board, boolean isWhite) {
        long key = 0;

        for (int i = 0; i < 64; i++) {
            key ^= getKeyForSquare(i, board.getPiece(i));
        }

        if (board.canCastleRight(true))
            key ^= whiteRightCastling;
        if (board.canCastleLeft(true))
            key ^= whiteLeftCastling;
        if (board.canCastleRight(false))
            key ^= blackRightCastling;
        if (board.canCastleLeft(false))
            key ^= blackLeftCastling;
